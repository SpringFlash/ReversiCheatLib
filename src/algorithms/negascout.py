"""
Модуль для реализации алгоритма NegaScout (Principal Variation Search)
"""
import time
import random
from src.core.heuristics import ReversiHeuristics

class NegaScoutAlgorithm:
    """
    Класс для реализации алгоритма NegaScout (улучшенная версия NegaMax)
    для игры Реверси.
    """
    def __init__(self):
    ...
    def get_best_move(self, board, player, max_depth=None):
        """
        Находит лучший ход с использованием алгоритма NegaScout
        ...
        if max_depth is None:
            ...
            if empty_count <= 12:  # Эндшпиль
                max_depth = 9  # Увеличиваем глубину для эндшпиля
            elif empty_count <= 20:  # Поздняя середина
        ...
        # Сортируем ходы для оптимизации поиска
        ...
        while max_search_depth <= max_depth and time.time() - self.start_time < self.max_time:
            # Инициализация для алгоритма NegaScout
            ...
            # Для первого хода выполняем полный поиск
            first_move = sorted_moves[0]
            row, col = first_move
            board_copy = board.clone()
            board_copy.make_move(row, col, player)
            score = -self.negascout(board_copy, max_search_depth - 1, -beta, -alpha, opponent, game_phase)
            
            current_best_score = score
            current_best_move = {"row": row, "col": col, "score": score}
            alpha = max(alpha, score)
            
            # Для остальных ходов используем нулевое окно
            for move in sorted_moves[1:]:
                ...
                # Сначала выполняем поиск с нулевым окном (это быстрее)
                score = -self.negascout(board_copy, max_search_depth - 1, -alpha - 0.1, -alpha, opponent, game_phase)
                
                # Если оценка хода выше текущей альфы, выполняем полный поиск
                if alpha < score < beta:
                    score = -self.negascout(board_copy, max_search_depth - 1, -beta, -score, opponent, game_phase)
        ...
        # print(f"NegaScout: глубина={max_search_depth-1}, узлы={self.nodes_visited}, time={elapsed:.3f}s, TT hits={self.tt_hits}")
        
        return best_move
    
    def negascout(self, board, depth, alpha, beta, player, game_phase):
        """
        Рекурсивная функция алгоритма NegaScout
        ...
        if not valid_moves:
            ...
            return -self.negascout(board, depth - 1, -beta, -alpha, opponent, game_phase)
        
        # Сортируем ходы для улучшения эффективности
        ...
        # Первый ребенок исследуется с полным окном
        first_move = sorted_moves[0]
        row, col = first_move
        board_copy = board.clone()
        board_copy.make_move(row, col, player)
        ...
        best_score = -self.negascout(board_copy, adaptive_depth - 1, -beta, -alpha, opponent, game_phase)
        
        if best_score > alpha:
            entry_type = "exact"  # Точная оценка
            alpha = best_score
        
        # Если отсечение, возвращаем лучшую оценку
        if alpha >= beta:
            # Сохраняем нижнюю границу, так как поиск был отсечен
            self.transposition_table[tt_key] = {"score": best_score, "depth": depth, "type": "lower_bound"}
            return best_score
        
        # Остальные дети исследуются с нулевым окном
        for move in sorted_moves[1:]:
            # Проверяем, не истекло ли время
            if time.time() - self.start_time >= self.max_time:
                break
            ...
            # Адаптивная глубина для угловых ходов
            adaptive_depth = depth
            if (row, col) in corners:
                adaptive_depth += 1
                
            # Нулевое окно поиска
            score = -self.negascout(board_copy, adaptive_depth - 1, -alpha - 0.1, -alpha, opponent, game_phase)
            
            # Если оценка хода выше текущей альфы, но ниже беты, выполняем полный поиск
            if alpha < score < beta:
                score = -self.negascout(board_copy, adaptive_depth - 1, -beta, -alpha, opponent, game_phase)
